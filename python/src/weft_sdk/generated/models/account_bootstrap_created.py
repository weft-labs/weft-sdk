...
class AccountBootstrapCreated(BaseModel):
    ...
    device_code: Annotated[str, Field(min_length=32, strict=True)] = Field(description="Returned once. Poll the OAuth token endpoint with this secret.")
    ...
    def status_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['pending', 'claimed', 'rejected', 'expired', 'consumed']):
            raise ValueError("must be one of enum values ('pending', 'claimed', 'rejected', 'expired', 'consumed')")
        return value
    ...
    def capabilities_validate_enum(cls, value):
        """Validates the enum"""
        for i in value:
            if i not in set(['search', 'status', 'cancel']):
                raise ValueError("each list item must be one of ('search', 'status', 'cancel')")
        return value
